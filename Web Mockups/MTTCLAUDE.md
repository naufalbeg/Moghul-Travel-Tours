...
the source of truth for requirements and detailed design; this file is the quick
orientation, not a replacement for them.
...
Supabase (migration `20260924202507_init` already applied — do not regenerate
from scratch). Nine tables: `users`, `packages`, `package_images`, `inquiries`,
`gallery_images`, `testimonials`, `site_config`, `announcements`, `audit_log`.
Full field-level detail is in SDD Section 4.5 (Data Dictionary) if needed —
the schema.prisma file itself is the authoritative, current version.
...
- **Colors:** Primary blue `#1B5FA8`, darker blue `#123A66`, orange accent
  `#F07C1E`, warm ivory background `#F5F9FC`, near-black text (not pure
  black) `#1A2733`
...
- Full HTML/CSS reference mockups exist for: homepage, login, admin dashboard,
  user management, add-admin modal, login error states, package listing,
  package detail, admin package list, package form, form validation errors —
  these are pixel-accurate references for matching styling, not just
  inspiration
...
ambiguous — check there before guessing at edge-case behavior.
...
1. Design system foundation — Tailwind config matching the color/font tokens
   above, shared layout components (header, footer, admin sidebar)
